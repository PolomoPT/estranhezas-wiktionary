...
# Connect
try:
    # Get all pages with the categories (lemma assumed) 
    with connection.cursor(dictionary=True) as cursor:
        ...
        AND cl1.cl_to = "Portuguese_nouns"
        AND cl2.cl_to = "Portuguese_past_participle_forms"
        ORDER BY page_title;"""

        cursor.execute(sql)
        results = cursor.fetchall()
        with open("substantivos\\lista.txt", "w", encoding="utf-8") as file:
